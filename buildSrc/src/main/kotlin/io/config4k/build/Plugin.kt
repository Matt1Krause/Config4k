package io.config4k.build

import io.config4k.build.Config4kBuildExtension.Target
...
class Config4kBuildExtension(properties: Map<in String, *>) {
    ...
    } ?: listOf(Target.JVM, Target.JS, Target.NATIVE)

    var dependOnCommon: Boolean = properties["config4k.dependOnCommon"]?.cast<String>()?.toBoolean() ?: true
    var publish: Publish? =
        if (properties.containsKey("config4k.publish") ||
            properties.entries.any { it.key?.tryCast<String>()?.startsWith("config4k.publish") == true }
        )
            Publish(properties)
        else
            null
